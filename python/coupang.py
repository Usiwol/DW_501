...
db_colum=['색상','소재','모델착용','세탁팁','사이즈','어깨단면','어깨길이',
...
for name in item_list:
    ...
    i=0
    for col in db_colum:    #가져온 상품 명칭이 db컬럼과 일치하는 것만 고랄서 값 넣어주기 위한 것
        if col in item_title[i]:   #실제 상품정보의 명칭이 db컬럼과 일지하면 값 저장
            item_value.append(item_list[name][item_title[i]])
            i+=1
        else:   #실제 상품정보의 명칭이 db컬럼과 일치하지 않으면 0 저장
            item_value.append(0)
    #print(item_value)
